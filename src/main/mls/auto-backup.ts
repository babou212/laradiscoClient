...
export function startAutoBackup(): void {
    ...
    backupTimer = setInterval(async () => {
        ...
        for (const serverId of dirtyIds) {
            ...
            try {
                // Trigger the auto-update via the existing IPC handler
                // which encrypts with the cached key and returns the payload
                const result = await ipcMain.emit('mls:autoUpdateBackup', serverId);
                // The actual upload to server happens from the renderer via the store
                // So we just emit an event the renderer can listen for
                if (result) {
