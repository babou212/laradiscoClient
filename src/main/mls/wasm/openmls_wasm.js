...
export class Group {
     ...
     * @param {Provider} provider
     */
    approve_staged_commit(provider) {
        _assertClass(provider, Provider);
        const ret = wasm.group_approve_staged_commit(this.__wbg_ptr, provider.__wbg_ptr);
        if (ret[1]) {
     ...
     */
    discard_staged_commit() {
        const ret = wasm.group_discard_staged_commit(this.__wbg_ptr);
        if (ret[1]) {
            throw takeFromExternrefTable0(ret[0]);
        }
    }
     ...
     * @returns {boolean}
     */
    has_pending_staged_commit() {
        const ret = wasm.group_has_pending_staged_commit(this.__wbg_ptr);
        return ret !== 0;
    }
    /**
     * @returns {boolean}
...
export class KeyPackage {
     ...
     */
    credential_identity() {
        const ret = wasm.keypackage_credential_identity(this.__wbg_ptr);
        var v1 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
        wasm.__wbindgen_free(ret[0], ret[1] * 1, 1);
        return v1;
    }
...
export class ProcessedMessage {
     ...
     */
    get staged_commit_json() {
        const ret = wasm.processedmessage_staged_commit_json(this.__wbg_ptr);
        let v1;
        if (ret[0] !== 0) {
...
const UpdateMessagesFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
