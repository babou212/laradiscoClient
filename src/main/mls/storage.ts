...
export function snapshotProviderState(serverId: number): Buffer | null {
    const db = getDb();
    const row = db
        .select({ provider_bytes: mlsProviderState.provider_bytes })
        .from(mlsProviderState)
        .where(eq(mlsProviderState.server_id, serverId))
        .get();
    return row ? row.provider_bytes : null;
}

export function restoreProviderSnapshot(serverId: number, snapshot: Buffer | null): void {
    const db = getDb();
    if (snapshot === null) {
