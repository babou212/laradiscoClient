...
export class MlsEpochConflictError extends Error {
    readonly currentEpoch: number;
    readonly expectedEpoch: number;
    constructor(currentEpoch: number, expectedEpoch: number, message?: string) {
        super(message ?? `MLS epoch conflict: server current_epoch=${currentEpoch}, expected=${expectedEpoch}`);
        this.name = 'MlsEpochConflictError';
...
export async function postGroupMessage(
    ...
    try {
        await api.post(`/e2ee/mls/groups/${encodeURIComponent(groupId)}/messages`, data);
    } catch (err) {
        if (
            axios.isAxiosError(err) &&
            err.response?.status === 409 &&
            err.response.data?.code === 'mls_epoch_conflict'
        ) {
            throw new MlsEpochConflictError(
                Number(err.response.data.current_epoch ?? 0),
                Number(err.response.data.expected_epoch ?? 0),
