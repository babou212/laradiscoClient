...
async function onAvatarSave(blob: Blob) {
    try {
        ...
        if (response.data) {
            usersStore.upsert({
                id: response.data.id,
                avatar_urls: response.data.attributes.avatar_urls ?? null,
            });
            if (authStore.user?.id === response.data.id) {
                authStore.user = {
                    ...authStore.user,
                    avatar_urls: response.data.attributes.avatar_urls ?? null,
                };
            ...
            <div class="flex items-center gap-6 p-6">
                ...
                <div class="flex flex-col gap-2">
                    <Button size="sm" @click="showAvatarDialog = true">
                        {{
                            authStore.user && usersStore.avatarUrl(authStore.user.id)
                                ? t('settings.profile.avatar.change')
                                : t('settings.profile.avatar.upload')
                        }}
                    </Button>
                    <Button
                        v-if="authStore.user && usersStore.avatarUrl(authStore.user.id)"
                        variant="ghost"
                        size="sm"
