...
function sanitizeLinkPreview(raw: unknown): LinkPreviewData | undefined {
    ...
    const imageUrl = typeof p.image_url === 'string' ? p.image_url.slice(0, 2048) : undefined;
    return {
        ...
        image,
        image_url: imageUrl && /^https:\/\//i.test(imageUrl) ? imageUrl : undefined,
...
export const mlsTrapNotice = ref<string | null>(null);

function isMlsTrapError(err: unknown): boolean {
    return err instanceof Error && err.name === 'MlsTrapError';
}

function noteTrap(action: string, err: unknown): void {
    if (isMlsTrapError(err)) {
        mlsTrapNotice.value = `${action} failed and was rolled back — please retry.`;
        console.error(`[E2EE] WASM trap rolled back during ${action}:`, err);
    }
}

function clearTrapNotice(): void {
    mlsTrapNotice.value = null;
}

async function withTrapReport<T>(action: string, fn: () => Promise<T>): Promise<T> {
    try {
        const result = await fn();
        if (mlsTrapNotice.value) clearTrapNotice();
        return result;
    } catch (err) {
        noteTrap(action, err);
        throw err;
...
export function useE2EE() {
    ...
    /**
     * Verify a KeyPackage's embedded credential identity matches the expected
     * userId/deviceId before adding the holder to a group. This closes the
     * RFC 9420 §5.3.1 application-credential-validation gap: openmls only
     * checks that the KeyPackage is well-formed and self-signed; the application
     * is responsible for binding the credential to the expected identity.
     */
    async function validateKeyPackageIdentity(
        keyPackageBytes: string,
        expectedUserId: number,
        expectedDeviceId: string,
    ): Promise<void> {
        const inspected = await window.api.mls.inspectKeyPackage({ keyPackageBytes });
        if (inspected.userId !== expectedUserId || inspected.deviceId !== expectedDeviceId) {
            throw new Error(
                `KeyPackage credential identity mismatch: expected ${expectedUserId}:${expectedDeviceId}, got ${inspected.identity}`,
            );
        }
    }

    /**
     * Resolve the set of user IDs eligible to be added to the given group.
     * Used to validate incoming MLS commits — adds for unknown users are
     * rejected before being merged into the group.
     */
    async function resolveAllowedAddUserIds(groupId: string): Promise<number[]> {
        let targetId: number;
        let type: 'channel' | 'dm';
        if (groupId.startsWith('channel:')) {
            targetId = Number(groupId.slice(8));
            type = 'channel';
        } else if (groupId.startsWith('dm:')) {
            targetId = Number(groupId.slice(3));
            type = 'dm';
        } else {
            return [];
        }
        ...
        try {
            const bundles = await getMemberBundles(targetId, type);
            return bundles.map((b) => b.user_id);
        } catch (err) {
            console.warn('[E2EE] Failed to resolve allowed add user IDs for', groupId, err);
            return [];
        }
    }
    ...
    async function submitCommitWithRetry(
        serverId: number,
        groupId: string,
        build: () => Promise<{ commit: string; welcome?: string; epoch: number }>,
        maxRetries = 3,
    ): Promise<{ commitEpoch: number; ratchetTree: string; welcome?: string }> {
        let attempt = 0;
        let staged = await build();
        while (true) {
            try {
                await postGroupMessage(groupId, {
                    device_id: await getDeviceId(),
                    message_type: 'commit',
                    message_bytes: staged.commit,
                    epoch: staged.epoch,
                });
                const mergeResult = await window.api.mls.mergeCommit({ serverId, groupId });
                return { commitEpoch: staged.epoch, ratchetTree: mergeResult.ratchetTree, welcome: staged.welcome };
            } catch (err) {
                if (!(err instanceof MlsEpochConflictError) || attempt >= maxRetries) {
                    await window.api.mls.clearPendingCommit({ serverId, groupId }).catch(() => {});
                    throw err;
                }
                attempt++;
                console.warn(
                    `[E2EE] commit rejected (epoch conflict, attempt ${attempt}/${maxRetries}), catching up and replaying`,
                    { groupId, currentEpoch: err.currentEpoch, expected: err.expectedEpoch },
                );
                await window.api.mls.clearPendingCommit({ serverId, groupId }).catch(() => {});
                await catchUpGroup(groupId, err.currentEpoch);
                staged = await build();
            }
        }
    }
        ...
        for (const member of members) {
            for (const device of member.devices) {
                ...
                try {
                    ...
                    const { ratchetTree, welcome } = await submitCommitWithRetry(serverId, groupId, async () => {
                        const freshPackages = await getKeyPackages(member.user_id, device.device_id);
                        const freshKp = freshPackages.find((p) => p.device_id === device.device_id) ?? freshPackages[0];
                        if (!freshKp?.key_package_bytes) {
                            throw new Error(`No key package available for ${member.user_id}/${device.device_id}`);
                        }
                        await validateKeyPackageIdentity(freshKp.key_package_bytes, member.user_id, device.device_id);
                        const added = await window.api.mls.addMember({
                            serverId,
                            groupId,
                            keyPackageBytes: freshKp.key_package_bytes,
                            expectedUserId: member.user_id,
                            expectedDeviceId: device.device_id,
                        });
                        return { commit: added.commit, welcome: added.welcome, epoch: added.epoch };
                    });
                    ...
                    await sendWelcome(groupId, {
                        ...
                        ratchet_tree_bytes: ratchetTree,
                    });
    ...
    async function catchUpGroup(groupId: string, currentEpoch: number): Promise<boolean> {
        ...
        try {
            ...
            const ourDeviceId = await getDeviceId();
            const allowedAddUserIds = await resolveAllowedAddUserIds(groupId);
            ...
            for (const msg of messages) {
                ...
                try {
                    ...
                    if (errMsg.includes('rejected by policy')) {
                        // A peer attempted to add an unauthorized member. The commit
                        // has been discarded locally; the group is now out of sync
                        // with whoever applied it. Abort and let upstream divergence
                        // recovery handle it (delete + rejoin), but don't silently retry.
                        console.error(
                            '[E2EE] MLS commit rejected by policy — possible attempted member injection:',
                            error,
                        );
                        return false;
                    }
    ...
    async function encryptForChannel(
        ...
        await enqueueGroupOp(channelId, undefined, async () => {
            ...
            result = await withTrapReport('Send message', () =>
                window.api.mls.encrypt({ serverId, groupId, plaintext: envelope }),
            );
        });
    ...
    async function decrypt(
        ...
        const result = await withTrapReport('Decrypt message', () =>
            window.api.mls.decrypt({
                serverId,
                groupId,
    ...
    async function lookupDecryptedCache(messages: MessageData[]): Promise<void> {
        ...
        const replyLookup = messages
            .map((m) => m.reply_to)
            .filter(
                (r): r is NonNullable<MessageData['reply_to']> =>
                    !!r && r.decrypted_content === undefined && !r.decrypt_error,
            );
        const threadLookup = messages
            .map((m) => m.thread?.last_reply)
            .filter(
                (r): r is NonNullable<NonNullable<MessageData['thread']>['last_reply']> =>
                    !!r && r.decrypted_content === undefined && !r.decrypt_error,
            );
        if (needLookup.length === 0 && replyLookup.length === 0 && threadLookup.length === 0) return;

        const ids = Array.from(
            new Set([
                ...needLookup.map((m) => Number(m.id)),
                ...replyLookup.map((r) => Number(r.id)),
                ...threadLookup.map((r) => Number(r.id)),
            ]),
        );
        try {
            ...
            for (const r of replyLookup) {
                const pt = cached[Number(r.id)];
                if (pt != null) {
                    const payload = deserializeFromCache(pt);
                    r.decrypted_content = payload.text;
                    r.link_preview = payload.link_preview ?? null;
                    r.decrypt_error = false;
                }
            }
            for (const r of threadLookup) {
                const pt = cached[Number(r.id)];
                if (pt != null) {
                    const payload = deserializeFromCache(pt);
                    r.decrypted_content = payload.text;
    ...
    async function decryptNestedPreviews(
        messages: MessageData[],
        channelId?: number,
        dmGroupId?: number,
    ): Promise<void> {
        for (const m of messages) {
            if (m.reply_to && m.reply_to.decrypted_content === undefined && !m.reply_to.decrypt_error) {
                await decryptReplyTo(m.reply_to, channelId, dmGroupId);
            }
            if (
                m.thread?.last_reply &&
                m.thread.last_reply.decrypted_content === undefined &&
                !m.thread.last_reply.decrypt_error
            ) {
                await decryptThreadLastReply(m.thread.last_reply, channelId, dmGroupId);
            }
        }
    }
    ...
    async function decryptMessages(messages: MessageData[], channelId?: number, dmGroupId?: number): Promise<void> {
        return enqueueDecryption(channelId, dmGroupId, async () => {
            ...
            await decryptNestedPreviews(messages, channelId, dmGroupId);
        });
    }

    async function decryptMessageQueued(message: MessageData, channelId?: number, dmGroupId?: number): Promise<void> {
        return enqueueDecryption(channelId, dmGroupId, async () => {
            await lookupDecryptedCache([message]);
            if (message.decrypted_content === undefined && !message.decrypt_error) {
                await decryptMessage(message, channelId, dmGroupId);
            }
            await decryptNestedPreviews([message], channelId, dmGroupId);
        });
    }
    ...
    async function retryPendingDecryptions(
        ...
        const hasPendingNested = messages.some(
            (m) =>
                (m.reply_to && m.reply_to.decrypted_content === undefined && !m.reply_to.decrypt_error) ||
                (m.thread?.last_reply &&
                    m.thread.last_reply.decrypted_content === undefined &&
                    !m.thread.last_reply.decrypt_error),
        );
        if (pending.length === 0 && !hasPendingNested) return;
        ...
        for (const groupId of groupIds) {
            ...
            try {
                await enqueueGroupOp(channelId, dmGroupId, async () => {
                    ...
                    const { ratchetTree, welcome } = await submitCommitWithRetry(serverId, groupId, async () => {
                        const refreshed = await getKeyPackages(userId, newDeviceId);
                        const freshKp = refreshed.find((p) => p.device_id === newDeviceId) ?? refreshed[0] ?? kp;
                        if (!freshKp?.key_package_bytes) {
                            throw new Error(`No key package available for device ${newDeviceId}`);
                        }
                        await validateKeyPackageIdentity(freshKp.key_package_bytes, userId, newDeviceId);
                        const added = await window.api.mls.addMember({
                            serverId,
                            groupId,
                            keyPackageBytes: freshKp.key_package_bytes,
    ...
    async function handleJoinRequest(
        ...
        await enqueueGroupOp(channelId, dmGroupId, async () => {
            ...
            const { ratchetTree, welcome } = await submitCommitWithRetry(serverId, groupId, async () => {
                const refreshed = await getKeyPackages(requesterUserId, requesterDeviceId);
                const freshKp = refreshed.find((p) => p.device_id === requesterDeviceId) ?? refreshed[0] ?? keyPackage;
                if (!freshKp?.key_package_bytes) {
                    throw new Error(`No key package for join requester ${requesterUserId}/${requesterDeviceId}`);
                }
                await validateKeyPackageIdentity(freshKp.key_package_bytes, requesterUserId, requesterDeviceId);
                const added = await window.api.mls.addMember({
                    serverId,
                    groupId,
                    keyPackageBytes: freshKp.key_package_bytes,
                    expectedUserId: requesterUserId,
                    expectedDeviceId: requesterDeviceId,
                });
                return { commit: added.commit, welcome: added.welcome, epoch: added.epoch };
            });
