...
<template>
    ...
    >
        <Avatar v-if="thread.last_reply?.user" class="size-5 shrink-0">
            <AvatarImage
                v-if="usersStore.avatarUrl(thread.last_reply.user.id, 'thumb')"
                :src="usersStore.avatarUrl(thread.last_reply.user.id, 'thumb')!"
                :alt="thread.last_reply.user.username"
            />
            <AvatarFallback class="bg-primary text-primary-foreground text-[10px] font-semibold">
                {{
                    usersStore.displayName(thread.last_reply.user.id, thread.last_reply.user.username)[0].toUpperCase()
                }}
            </AvatarFallback>
        </Avatar>
