...
<template>
    <div v-if="voiceStore.isConnected" class="border-sidebar-border border-t bg-black/20 px-3 pt-2 pb-1">
        ...
        <button
            v-if="voiceStore.isAudioPlaybackBlocked"
            type="button"
            class="mb-2 flex h-7 w-full items-center justify-center rounded bg-yellow-500/20 text-xs font-semibold text-yellow-300 transition-colors hover:bg-yellow-500/30"
            @click="voiceStore.enableAudioPlayback()"
        >
