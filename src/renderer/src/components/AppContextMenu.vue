...
function dispatchMessageAction(action: string, detail?: Record<string, unknown>) {
    ...
    if (!ctx) return;
    const el = ctx.el;
    // Defer to a macrotask so the originating click event has finished bubbling
    // before we mutate state. Otherwise listeners like MessagesPanel's
    // click-outside handler run after our state change and undo it.
    setTimeout(() => {
        el.dispatchEvent(
